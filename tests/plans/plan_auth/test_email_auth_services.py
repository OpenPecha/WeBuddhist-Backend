...
from pecha_api.auth.auth0_email import Auth0EmailIdentity
from pecha_api.plans.auth.plan_auth_enums import AuthorStatus
from pecha_api.plans.auth.plan_auth_models import EmailExchangeRequest, TokenResponse
from pecha_api.plans.auth.plan_auth_services import exchange_email_token
...
def test_exchange_creates_verified_inactive_email_profile(
    ...
    assert author_arg.created_by == "auth0_email:auth0|123"
...
def test_exchange_new_profile_requires_both_names():
    ...
    with patch(
        "pecha_api.plans.auth.plan_auth_services.verify_auth0_email_token",
        return_value=email_identity,
    ), patch(
        ...
        with pytest.raises(HTTPException) as exc:
            exchange_email_token(
                EmailExchangeRequest(auth0_token="auth0-token", first_name="Ada")
            )
...
def test_exchange_existing_active_author_returns_tokens(
    ...
    with patch(
        ...
        response = exchange_email_token(
            EmailExchangeRequest(auth0_token="auth0-token")
        )
...
def test_exchange_first_verification_of_existing_author_notifies_pending_invites(
    ...
    with patch(
        ...
        exchange_email_token(EmailExchangeRequest(auth0_token="auth0-token"))

...
def _build_notification_copy(*, author_name: str, caption: str | None) -> str:
    if not caption or not caption.strip():
        return f"{author_name} shared a new post"
...
def get_group_post_notification_targets(
    *,
    ...
    with SessionLocal() as db:
        ...
        author_name = f"{author.firstname} {author.lastname or ''}".strip() or author.email
        title = get_group_notification_title(db, post.group_id)
        body = _build_notification_copy(author_name=author_name, caption=post.caption)
